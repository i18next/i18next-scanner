...
var debuglog = util.debuglog(pkg.name);
    ...
    _.each(options.lngs, function(lng) {
        ...
        _.each(options.ns.namespaces, function(ns) {
            var regex = {
                ...
                'ns': new RegExp(_.escapeRegExp(options.interpolationPrefix + 'lng' + options.interpolationSuffix), 'g')
            };
    ...
    _.each(params, function(param) {
        ...
        var value = (function(value) { // value is enclosed with either single quote (') or double quote (") characters
            ...
            if (quoteChar) { // single quote (') or double quote (")
                value = _.unquote(value, quoteChar);
            }
...
Parser.prototype.write = function() {
    ...
    _.each(options.lngs, function(lng) {
        _.each(options.ns.namespaces, function(ns) {
            ...
            // Sorted by key
            resStore[lng][ns] = _.chain(resStore[lng][ns])
                .pairs()
            ...
            var filepath = path.join(__dirname, '..', '..', resPath),
                content = JSON.stringify(resStore[lng][ns], null, 4); // Indented 4 spaces  

            fs.writeFileSync(filepath, content + '\n');

            debuglog('Saved ' + resPath);
        });
    });
};
